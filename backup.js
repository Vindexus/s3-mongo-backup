...
function BackupMongoDatabase(config) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const backupName = `${database}_${currentTime(timezoneOffset)}`
    const backupFilePath = backupName + '.zip'
    const dbPath = BACKUP_PATH(backupName)
    const zipPath = dbPath + '.zip'

    // Default command, does not considers username or password
    let command = `mongodump -h ${host} --port=${port} -d ${database} --quiet -o ${dbPath}`;

    // When Username and password is provided
    if (username && password) {
      command = `mongodump -h ${host} --port=${port} -d ${database} -p ${password} -u ${username} --quiet -o ${dbPath}`;
    }
    // When Username is provided
    if (username && !password) {
      command = `mongodump -h ${host} --port=${port} -d ${database} -u ${username} --quiet -o ${dbPath}`;
    }
